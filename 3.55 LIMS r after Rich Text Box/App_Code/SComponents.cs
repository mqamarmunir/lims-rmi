...
//{
	...
	public class SComponents
	{
		...
		public void FillListBox(ListBox lb,DataView dv,string strTextField,string strValueField,bool sort)
		{
		...
		{	
			...
			if(TDays < 31)
			{
				return ((int)(TDays / 7)).ToString() + " week(s)";
			}
			else if(TDays < 365)
			{
			...
			{
				return ((int)(TDays / 365)).ToString() + " year(s)";
			}
		}


		public DataView SetAgeInDataGrid(DataView dv, string strDOBColumn)
		{
			foreach(DataRow dr in dv.Table.Rows)
			{
				if(dr[strDOBColumn].ToString() != "")
				{
					dr["Age"] = GetAge(dr[strDOBColumn].ToString());
				}
			}
		...
		/// <summary>
		/// Trim Left Zeros from a string
		/// </summary>
		/// <param name="str">string to be trimmed</param>
		/// <param name="strLen">string length</param>
		/// <returns>trimmed string</returns>
		public string TrimLeftZeros(string str, int strLen)
		{
			char[] arrChange = str.ToCharArray();
			int index = 0;
		...
		/// <summary>
		/// Add Column "Age" first.
		/// </summary>
		/// <param name="dvLocal"></param>
		/// <param name="dobColumn"></param>
		/// <returns></returns>
		...
		public DataView DataViewLeftPading(DataView dv,string sourceColumnName,string targetColumnName,int strLen)
		{
			dv.Table.Columns.Add(targetColumnName);

			int dvCount = dv.Count;
			
			for(int counter = 0; counter < dvCount; counter++)
			{
				dv.Table.Rows[counter][targetColumnName] = dv.Table.Rows[counter][sourceColumnName].ToString().PadLeft(strLen, '0');
			}
		...
		public DataView DataViewPveNve(DataView dv,string sourceColumnName, string targetColumnName)
		{
			dv.Table.Columns.Add(targetColumnName);
			...
			for(int counter = 0; counter < dvCount; counter++)
			{
				if(dv.Table.Rows[counter][sourceColumnName].ToString() == "A")
				{
					dv.Table.Rows[counter][targetColumnName] = "-ve";
				}
				else if(dv.Table.Rows[counter][sourceColumnName].ToString() == "R")
				{
					dv.Table.Rows[counter][targetColumnName] = "+ve";
				}
			}
