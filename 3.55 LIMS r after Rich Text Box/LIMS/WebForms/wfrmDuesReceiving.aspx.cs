...
{
	/// <summary>
	/// Summary description for wfrmDuesReceiving.
	/// </summary>
	public partial class wfrmDuesReceiving : System.Web.UI.Page
	{
		...
		private void FillDG()
		{
			...
			DataView dvMisc = objMisc.GetAll(2);
		...
		private void rbtnlArchiver_SelectedIndexChanged(object sender, System.EventArgs e)
		{				
			FillDG();
		}
		...
		private void dgResultDis_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			else if(e.CommandName.Equals("Received"))
			{		
				
				clsBLDuesReceived objTDuesReceived = new clsBLDuesReceived();					
				objTDuesReceived.MSerialNo = e.Item.Cells[0].Text;				
				objTDuesReceived.PaidAmount = e.Item.Cells[6].Text;								
				objTDuesReceived.EntryDateTime = System.DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt");
				objTDuesReceived.EntryPerson = Session["loginid"].ToString();
				objTDuesReceived.Shift = Session["Shift"].ToString();			

				if (!objTDuesReceived.Insert())
				{
					lblErrMsg.Text = objTDuesReceived.ErrorMessage;
				...
				else
				{
					...
					objTMTransaction.PaidAmount = e.Item.Cells[10].Text;								
					if (!objTMTransaction.UpdateDues()) 
					{
						lblErrMsg.Text = objTMTransaction.ErrorMessage;
					} 
					else
					{
						FillDG();														
		...
		private void rbtnStatus_SelectedIndexChanged(object sender, System.EventArgs e)
		{
